...
import static com.immrhy.learningandroidapptool.fermata.media.pref.MediaPrefs.MEDIA_ENG_YT;
...
import com.immrhy.learningandroidapptool.fermata.addon.web.R;
import com.immrhy.learningandroidapptool.fermata.addon.web.yt.YoutubeAddon.VideoScale;
import com.immrhy.learningandroidapptool.fermata.media.engine.MediaEngine;
import com.immrhy.learningandroidapptool.fermata.media.lib.ExtPlayable;
...
class YoutubeMediaEngine implements MediaEngine, OverlayMenu.SelectionHandler {
	...
	public void contributeToMenu(OverlayMenu.Builder b) {
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.video_scaling,
				ResourcesCompat.getDrawable(r, R.drawable.video_scaling, ctx.getTheme()),
				r.getString(com.immrhy.learningandroidapptool.fermata.R.string.video_scaling)).setSubmenu(this::videoScalingMenu);
	}
		...
		return web.getVideoQualities().timeout(1100).main()
				...
				.map(qualities -> {
					if ((qualities == null) || (qualities.isEmpty())) {
						b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.auto, null, com.immrhy.learningandroidapptool.fermata.R.string.auto)
	...
	private void videoScalingMenu(OverlayMenu.Builder b) {
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_best, null, com.immrhy.learningandroidapptool.fermata.R.string.video_scaling_best)
				.setChecked(scale == VideoScale.CONTAIN, true);
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_fill, null, com.immrhy.learningandroidapptool.fermata.R.string.video_scaling_fill)
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_orig, null, com.immrhy.learningandroidapptool.fermata.R.string.video_scaling_orig)
	...
	public boolean menuItemSelected(OverlayMenuItem item) {
		...
		if (itemId == com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_best) {
			web.setScale(VideoScale.CONTAIN);
			return true;
		} else if (itemId == com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_fill) {
			web.setScale(VideoScale.FILL);
			return true;
		} else if (itemId == R.id.video_scaling_fill_proportional) {
			web.setScale(VideoScale.COVER);
			return true;
		} else if (itemId == com.immrhy.learningandroidapptool.fermata.R.id.video_scaling_orig) {
