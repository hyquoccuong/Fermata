package com.immrhy.learningandroidapptool.fermata.addon.web;

import static android.os.Build.*;
import static com.immrhy.learningandroidapptool.fermata.addon.web.FermataWebClient.isYoutubeUri;
...
import com.immrhy.learningandroidapptool.fermata.addon.web.yt.YoutubeFragment;
...
import com.immrhy.learningandroidapptool.fermata.ui.activity.VoiceCommand;
...
public class WebBrowserFragment extends MainActivityFragment
	...
	public int getFragmentId() {
		return com.immrhy.learningandroidapptool.fermata.R.id.web_browser_fragment;
	}
	...
	public void loadUrl(String url) {
		...
		if (v != null) {
			if (!(this instanceof YoutubeFragment) && isYoutubeUri(Uri.parse(url)) &&
					AddonManager.get().hasAddon(com.immrhy.learningandroidapptool.fermata.R.id.youtube_fragment)) {
				String u = url;
				MainActivityDelegate.getActivityDelegate(requireContext()).onSuccess(a -> {
					if (a.showFragment(com.immrhy.learningandroidapptool.fermata.R.id.youtube_fragment) instanceof YoutubeFragment f)
						f.loadUrl(u);
				});
			} else {
				v.loadUrl(url);
	...
	public void contributeToNavBarMenu(OverlayMenu.Builder b) {
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.refresh,
				ResourcesCompat.getDrawable(res, com.immrhy.learningandroidapptool.fermata.R.drawable.refresh, theme),
				res.getString(com.immrhy.learningandroidapptool.fermata.R.string.refresh)).setHandler(this);
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.bookmarks,
				ResourcesCompat.getDrawable(res, com.immrhy.learningandroidapptool.fermata.R.drawable.bookmark_filled, theme),
				res.getText(com.immrhy.learningandroidapptool.fermata.R.string.bookmarks)).setSubmenu(this::bookmarksMenu);
	}
	...
	public boolean menuItemSelected(OverlayMenuItem item) {
		...
		if (id == com.immrhy.learningandroidapptool.fermata.R.id.refresh) {
			v.reload();
			return true;
	...
	public void bookmarksMenu(OverlayMenu.Builder b) {
		...
		b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.bookmark_create, com.immrhy.learningandroidapptool.fermata.R.string.create_bookmark)
	...
	private void createBookmark(OverlayMenu.Builder b) {
		...
		set.addStringPref(o -> {
			o.store = store;
			o.pref = name;
			o.title = com.immrhy.learningandroidapptool.fermata.R.string.bookmark_name;
		});
		set.addStringPref(o -> {
			o.store = store;
			o.pref = url;
	...
	private boolean bookmarkSelected(OverlayMenuItem item) {
		if (item.isLongClick()) {
			...
			item.getMenu().show(b ->
					b.addItem(com.immrhy.learningandroidapptool.fermata.R.id.bookmark_remove, com.immrhy.learningandroidapptool.fermata.R.string.remove_bookmark)
							.setHandler(i -> {
								WebBrowserAddon a = getAddon();
