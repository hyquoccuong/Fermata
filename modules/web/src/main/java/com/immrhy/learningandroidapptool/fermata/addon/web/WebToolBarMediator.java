...
public class WebToolBarMediator implements ToolBarView.Mediator {
	...
	public void enable(ToolBarView tb, ActivityFragment f) {
		...
		addButton(tb, com.immrhy.learningandroidapptool.utils.R.drawable.back, v ->
				requireNonNull(b.getWebView()).goBack(), com.immrhy.learningandroidapptool.utils.R.id.tool_bar_back_button, LEFT);
		addButton(tb, R.drawable.clear, v -> t.setText(""), R.id.browser_addr_clear);
		addButton(tb, com.immrhy.learningandroidapptool.fermata.R.drawable.bookmark_filled, v ->
				onBookmarksButtonClick(b), com.immrhy.learningandroidapptool.fermata.R.id.bookmarks);
	...
	public void setButtonsVisibility(ToolBarView tb, boolean back, boolean forward) {
		tb.findViewById(com.immrhy.learningandroidapptool.utils.R.id.tool_bar_back_button).setVisibility(back ? VISIBLE : GONE);
