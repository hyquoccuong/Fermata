...
public class SubGenAddon implements FermataAddon {
	...
	public void contributeSettings(Context ctx, PreferenceStore ps, PreferenceSet set,
		...
		set.addBooleanPref(o -> {
			o.store = mps;
			o.pref = TRANSLATE;
			o.title = com.immrhy.learningandroidapptool.fermata.R.string.sub_gen_translate;
		...
		set.addButton(o -> {
			o.title = com.immrhy.learningandroidapptool.fermata.R.string.sub_gen_cleanup;
			o.onClick = () -> {
				...
				showInfo(ctx, ctx.getString(com.immrhy.learningandroidapptool.fermata.R.string.sub_gen_cleanup_done, deleted));
			};
			o.visibility = visibility.copy();
		});
	}
	...
	public FutureSupplier<Transcriptor> getTranscriptor(PreferenceStore ps) {
		...
		var ctx = App.get();
		ActivityBase.create(App.get(), "com.immrhy.learningandroidapptool.fermata.subgen", "subgen",
				com.immrhy.learningandroidapptool.fermata.R.drawable.notification, ctx.getString(R.string.downloading, "..."),
