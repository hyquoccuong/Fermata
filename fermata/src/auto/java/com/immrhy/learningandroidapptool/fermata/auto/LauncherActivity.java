...
import static com.immrhy.learningandroidapptool.fermata.ui.activity.MainActivityDelegate.INTENT_ACTION_FINISH;
...
import static com.immrhy.learningandroidapptool.utils.ui.activity.ActivityDelegate.FULLSCREEN_FLAGS;
...
import com.immrhy.learningandroidapptool.utils.ui.view.MovableRecyclerViewAdapter;
...
public class LauncherActivity extends AppCompatActivity {
	...
	private static final class AppListView extends RecyclerView {
		...
		public AppListView(@NonNull Context ctx) {
			super(ctx);
			apps = loadAppList();
			animation = AnimationUtils.loadAnimation(ctx, com.immrhy.learningandroidapptool.utils.R.anim.button_press);
			...
			void setAppInfo(AppInfo appInfo) {
				...
				if (appInfo == AppInfo.ADD) {
					...
					if (appInfo instanceof SelectAppInfo sai) {
						Drawable icon;
						if (sai.selected) {
							if (checkedIcon == null) checkedIcon = loadIcon(com.immrhy.learningandroidapptool.utils.R.drawable.check_box);
							icon = checkedIcon;
						} else {
							if (uncheckedIcon == null)
								uncheckedIcon = loadIcon(com.immrhy.learningandroidapptool.utils.R.drawable.check_box_blank);
							icon = uncheckedIcon;
						}
